...
var db = new Database(path.resolve(process.cwd(), "invoices.db"));
db.exec(`
...
`);
var addColumnIfNotExists = (table, column, type) => {
  try {
...
try {
  ...
  const rowsToFix = db.prepare("SELECT id, data FROM invoices WHERE total IS NULL OR total = 0").all();
...
app.get("/api/invoices/next-number/:type", authenticateToken, (req, res) => {
